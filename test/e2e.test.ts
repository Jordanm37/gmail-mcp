...
const streamableClientUrl = new URL(
...
describe("Gmail MCP", () => {
  ...
  describe("Stdio Transport", () => {
    ...
    beforeAll(async () => {
      ...
      stdioClient.onmessage = (message) =>
        readMessages.push(message as ReadMessageType);
    ...
    //     readMessages[0].result.content?.[0].text ?? "{}"
    //   );
    ...
    it("can call the list_drafts tool", async () => {
      ...
      const createdDraft = draftsList.find(
    ...
    it("can call the get_draft tool", async () => {
      const params = jsonRpcMessage.getDraft
    ...
    it("can call the delete_draft tool", async () => {
      const params = jsonRpcMessage.deleteDraft
        .params as CallToolRequest["params"];
      if (params?.arguments) params.arguments.id = draftId;
  ...
  describe("Streamable HTTP Transport", () => {
    ...
    // it("can call the get_profile tool", async () => {
    //   const params = jsonRpcMessage.getProfile
    //     .params as CallToolRequest["params"];
    ...
    it("can call the create_draft tool", async () => {
      const params = jsonRpcMessage.createDraft
        .params as CallToolRequest["params"];
      const response = await streamableClient.callTool(params);
    ...
    it("can call the list_drafts tool", async () => {
      const params = jsonRpcMessage.listDrafts
        .params as CallToolRequest["params"];
      const response = await streamableClient.callTool(params);
