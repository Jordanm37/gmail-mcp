...
const getEnvBasedCredentials = (queryConfig?: Record<string, any>) => {
  ...
  return { clientId, clientSecret, refreshToken, fullCredentials: null };
};
...
const getFileBasedCredentials = () => {
  ...
  if (fs.existsSync(GMAIL_CREDENTIALS_PATH)) {
    const gmailCredentialsFile = JSON.parse(
      fs.readFileSync(GMAIL_CREDENTIALS_PATH, "utf8")
    );
    refreshToken = gmailCredentialsFile?.refresh_token;
    fullCredentials = gmailCredentialsFile;
  }
...
export const createOAuth2Client = (queryConfig?: Record<string, any>) => {
  try {
    ...
    // Set full credentials if available (includes access_token, refresh_token, expiry_date, etc.)
  ...
  new Promise((resolve, reject) => {
    ...
    server.on("request", async (req, res) => {
      ...
      try {
        ...
        res.writeHead(200);
        res.end(
          `Authentication successful! Go to ${GMAIL_CREDENTIALS_PATH} to view your REFRESH_TOKEN. You can close this window.`
        );
        server.close();
        resolve(void 0);
      } catch (error: any) {
...
export const validateCredentials = async (oauth2Client: OAuth2Client) => {
  try {
    ...
    // Try to save refreshed credentials, but don't fail if we can't
    try {
      fs.writeFileSync(GMAIL_CREDENTIALS_PATH, JSON.stringify(tokens, null, 2));
    } catch (writeError: any) {
      // Non-fatal - credentials are still set in memory
    }
    return true;
  } catch (error: any) {
    return false;
